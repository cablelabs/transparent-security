...
ERROR: All the attempts to run CPP on $P4_REALPATH failed.
       ...
       If so, you need to specify these as part of P4PPFLAGS variable, e.g
...
EOF
    ...
    if [ $inc_tofino == "yes" ]; then
        ...
        if grep -q 'tofino/intrinsic_metadata\.p4' $P4_REALPATH; then
            ...
            AUTO_P4_VERSION=p4_14
            AUTO_P4_ARCHITECTURE=simple_switch
            p4_autodetected=1
        fi
    else
        cat <<EOF
...
EOF
    ...
    echo "Using Build Directory $P4_BUILD"
    echo -n "  Building $P4_NAME in \`build_dir ${P4_NAME}${P4_SUFFIX}\` ... "
    rm -rf $P4_BUILD
    rm -rf $P4_LOGS
...
EOF
    ...
    mkdir -p $GRAPHS_DIR
    echo -n "  Building $P4_NAME graphs in \`graphs_dir ${P4_NAME}${P4_SUFFIX}\` ... "

    if [ $P4_ARCHITECTURE == "tofino" ]; then
        extra_args="-D__TARGET_TOFINO__  -I$SDE_INSTALL/share/p4_lib \
...
case ${P4_ARCHITECTURE,,} in
    simple[-_]switch|ss) P4_ARCHITECTURE=simple_switch;;
    v1model|v1) P4_ARCHITECTURE=v1model;;
    tna|psa|tofino) P4_ARCHITECTURE=${P4_ARCHITECTURE,,};;
    "");;
    *) cat <<EOF
ERROR: Supported values for P4_ARCHITECTURE are:
          For P4_14 programs: tofino and siple_switch
          FOR P4_16 programs: tna and v1model
EOF
       exit 1;;
esac
